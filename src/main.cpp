...
enum class EventType { ARRIVAL, DEPARTURE }; //Define um tipo enumerado seguro para identificar eventos de chegada e saída na simulação.
...
struct Simulation {
    ...
    void init() {
        ...
        while (!q.empty()) q.pop();          // fila começa VAZIA
    ...
    void process_departure(const Event& e) {
        ...
        if (q.empty()) {
            ...
            double arrival_time_next = q.front(); q.pop();  // próximo da FILA
    ...
    void run() {
        ...
        while (completed < N_target && !fel.empty()) {
            Event e = fel.top(); fel.pop();                // consome do topo da FEL
            update_time_avg_stats(e.time);
            clock = e.time;

            if (e.type == EventType::ARRIVAL)  process_arrival(e);
            else                                process_departure(e);
        }
